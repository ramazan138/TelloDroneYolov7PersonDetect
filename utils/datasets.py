...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            # Read video
            self.mode = 'video'
            ret_val, img0 = TelloControlDrone.imgCap()
            if not ret_val:
                self.count += 1
                self.cap.release()
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            self.imgs[i] = TelloControlDrone.imgCap()  # guarantee first frame
    ...
    def update(self, index, cap):
        ...
        while cap.isOpened():
            ...
            self.imgs[index] = TelloControlDrone.imgCap()
            
            cap.grab()
            if n == 4:  # read every 4th frame
                success, im = cap.retrieve()
                # self.imgs[index] = im if success else self.imgs[index] * 0
                self.imgs[index] =self.imgs[index]
                n = 0
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        img = TelloControlDrone.imgCap()  # BGR
...
def extract_boxes(path='../coco/'):  # from utils.datasets import *; extract_boxes('../coco128')
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in img_formats:
            ...
            im = TelloControlDrone.imgCap()(str(im_file))[..., ::-1]  # BGR to RGB
            h, w = im.shape[:2]
